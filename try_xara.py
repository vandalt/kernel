...
# %%
niriss_model = xara.core.create_discrete_model(pmask, ppscale, mstep, binary=binary_model, tmin=0.4)
...
ax.imshow(pmask)
ax.plot(
    psz / 2 + niriss_model[:, 0] / ppscale,
    psz / 2 + niriss_model[:, 1] / ppscale,
    "b."
)
...
kpo_0.kpi.plot_pupil_and_uv(xymax=3.5, cmap=plt.cm.plasma_r, ssize=9, figsize=(10,5), marker='o')
plt.show()
...
for fname in files:
    ...
    # %%
    kpo1.extract_KPD_single_cube(cube, pscale, wl, target=hdr["TARGPROP"], recenter=True)
